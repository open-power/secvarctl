...
## USAGE:    
  It has 5 main commands   
    `./secvarctl -m host read [options] [variable]`    
    `./secvarctl -m host write [options] <variable> <file>`    
    `./secvarctl -m host validate [options] [fileType] <file>`  
    `./secvarctl -m host verify [options] -u {update Variables}`  
    `./secvarctl -m host generate <inputFormat>:<outputFormat> [OPTIONS] -i <inputFile> -o <outputFile` 
...
## SUB COMMAND USAGE:
    ...
    VALIDATE:
                 ./secvarctl -m host validate [options] <file type> <file> 
	REQUIRED:
