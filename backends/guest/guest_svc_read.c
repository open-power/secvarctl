 ...
 */
static int
variable_from_path (const char *path, uint8_t **esl_data, size_t *esl_data_len)
{
  int rc = SUCCESS;
 ...
 */
static int
get_variable_path (const char *path, const char *variable_name, char **variable_path)
{
  int len = 0;
  char *esl_data = "/data";
 ...
 */
static int
read_cert (const uint8_t *cert_data, const size_t cert_data_len, const int is_print_raw,
 ...
 */
static int
read_esl (const uint8_t *esl_data, const size_t esl_data_len, const int is_print_raw,
          const uint8_t *variable_name)
{
  int rc = SUCCESS;
 ...
 */
static int
read_auth (const uint8_t *auth_data, size_t auth_data_len, const int is_print_raw,
           const uint8_t *variable_name)
{
 ...
 */
static int
read_path (const char *path, const int is_print_raw, const char *variable_name)
{
  int rc;
  ...
  if (variable_name != NULL)
      ...
      if (rc == SUCCESS)
        {
          if (is_print_raw || esl_data_size == DEFAULT_PK_LEN)
            ...
            rc = print_variables (esl_data + TIMESTAMP_LEN, esl_data_size - TIMESTAMP_LEN,
  ...
  else
    {
      ...
      {
        rc = get_variable_path (path, (char *) defined_sb_variables[i], &variable_path);
        if (rc != SUCCESS)
          return rc;
...
guest_read_command (int argc, char *argv[])
{
  ...
  struct argp_option options[] = {
    ...
    { "path", 'p', "PATH",
      0, "looks for Guest secure boot variable directories in PATH, default is " SECVARPATH },
...
struct command guest_command_table[] = {
  { .name = "read", .func = guest_read_command },
