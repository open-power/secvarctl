 ...
 */
static void
convert_timestamp (timestamp_t *timestamp, struct tm *current_time)
{
 ...
 */
int
extract_esl_from_auth (const uint8_t *data, const size_t data_size, uint8_t **out, size_t *out_size)
{
 ...
 */
static int
create_prehash (const uint8_t *esl, const size_t esl_size, const struct generate_args *args,
                const uuid_t guid, uint8_t **out_data, size_t *out_data_size)
{
  int rc = SV_SUCCESS;
  uint8_t *ptr = NULL;
  uint8_t *wkey = NULL;
  size_t varlen;
  leint32_t attr = cpu_to_le32 (SECVAR_ATTRIBUTES |
                                (args->append_flag ? SV_VARIABLE_APPEND_WRITE : 0));
 ...
 */
int
create_presigned_hash (const uint8_t *esl, const size_t esl_size, const struct generate_args *args,
                       const uuid_t guid, uint8_t **out_buffer, size_t *out_buffer_size)
{
  int rc;
  ...
  rc = crypto.generate_md_hash (prehash, prehash_size, CRYPTO_MD_SHA256,
 ...
 */
int
create_pkcs7 (const uint8_t *new_data, const size_t new_data_size, const struct generate_args *args,
              const uuid_t guid, uint8_t **out_buffer, size_t *out_buffer_size)
{
  int rc;
  size_t total_size;
  ...
  if (args->pkcs7_gen_method)
    ...
    rc = crypto.generate_pkcs7_signature (actual_data, total_size, (uint8_t **) args->sign_certs,
                                          (uint8_t **) args->sign_keys, args->sign_key_count,
                                          out_buffer, out_buffer_size);
                 ...
                 uint8_t **out_buffer, size_t *out_buffer_size)
{
  ...
  auth_header.auth_cert.hdr.da_length = sizeof (auth_header.auth_cert.hdr) +
                                        sizeof (auth_header.auth_cert.cert_type) +
                                        pkcs7_size;
  ...
  *out_buffer_size = APPEND_HEADER_LEN + sizeof (auth_header) + pkcs7_size +
                     new_esl_size;
 ...
 */
int
is_x509certificate (const uint8_t *buffer, const size_t buffer_size, uint8_t **cert_data,
 ...
 */
int
get_hash_data (const uint8_t *buffer, const size_t buffer_size, hash_func_t **hash_funct,
               uint8_t *hash_data, size_t *hash_data_size)
{
  ...
  rc = crypto.generate_md_hash (data, data_size, (*hash_funct)->crypto_md_funct,
