 ...
 */
static int
validate_single_esl (const uint8_t *esl_data, size_t esl_data_size,
                     size_t *next_esl)
{
  ssize_t cert_size;
  int rc;
  ...
  *next_esl = 0;
  ...
  if (esl_data_size < sizeof (sv_esl_t))
    {
      prlog (PR_ERR, "ERROR: ESL has %zd bytes and is smaller than an ESL (%zd bytes),"
  ...
  if (sig_list->signature_list_size > esl_data_size ||
      sig_list->signature_header_size > esl_data_size || sig_list->signature_size > esl_data_size)
    {
      prlog (PR_ERR, "ERROR: expected signature list size %d + header size %d + "
  ...
  if (esl_size > esl_data_size)
    {
      prlog (PR_ERR, "ERROR: signature list size is greater than remaining data size: %zd > %zd\n",
...
validate_auth (const uint8_t *auth_data, size_t auth_data_len)
{
  ...
  if (auth_size == auth_data_len)
    ...
    {
      rc = validate_esl (auth_data + APPEND_HEADER_LEN + auth_size, auth_data_len - auth_size);
      if (rc)
        {
