 ...
 */
static bool
is_global_variable (const uint8_t *variable_name)
{
 ...
 */
static bool
is_sbat_variable (const uint8_t *variable_name)
{
  int len = strlen ((char *) variable_name);
 ...
 */
static int
generate_esl (const uint8_t *buffer, size_t buffer_size, struct generate_args *args,
              const hash_func_t *hash_funct, uint8_t **out_buffer, size_t *out_buffer_size)
{
  int rc = SUCCESS;
  ...
  switch (args->input_form[0])
    {
      ...
      case 'f':
        if (is_global_variable ((uint8_t *) args->variable_name))
          {
            prlog (PR_ERR, "ERROR: PK and KEK are not allowed to generate hash from file\n");
      ...
      case 'h':
        ...
        break;
      case 'c':
        if (is_x509certificate (buffer, buffer_size, &cert_data, &cert_data_size) != SUCCESS)
          {
      ...
      case 'a':
        ...
        break;
      default:
        prlog (PR_ERR, "ERROR: unknown input format %s for generating an ESL,"
 ...
 */
static int
generate_sha256_hash (const uint8_t *data, size_t data_size, struct generate_args *args,
  ...
  if (!args->input_valid)
    {
      switch (args->input_form[0])
        {
          ...
          case 'a':
            ...
            break;
          default:
            prlog (PR_ERR, "ERROR: unknown input format %s for generating a hash,"
                   " use `--help` for more info\n", args->input_form);
            rc = ARG_PARSE_FAIL;
        }
      ...
      if (rc)
        {
          prlog (PR_ERR,
                 "failed to validate input format of input file when "
                 "generating hash, "
 ...
 */
static int
generate_authorpkcs7 (const uint8_t *buffer, size_t buffer_size, struct generate_args *args,
                      const hash_func_t *hash_funct, uint8_t **out_buffer, size_t *out_buffer_size)
{
  int rc = SUCCESS;
  ...
  switch (args->input_form[0])
    {
      case 'r':
        ...
        else if (memcmp (PK_VARIABLE, args->variable_name, strlen (args->variable_name)) == 0)
          {
  ...
  if (args->output_form[0] == 'a')
    rc = create_auth_msg (*input_ptr, input_size, args, *guid, out_buffer, out_buffer_size);
  else if (args->output_form[0] == 'x')
    rc = create_presigned_hash (*input_ptr, input_size, args, *guid, out_buffer, out_buffer_size);
  else
 ...
 */
static int
generate_data (const uint8_t *buffer, size_t buffer_size, struct generate_args *args,
               const hash_func_t *hash_function, uint8_t **out_buffer, size_t *out_buffer_size)
{
  int rc = SUCCESS;
  ...
  switch (args->output_form[0])
    {
      ...
      case 'h':
        ...
        rc = generate_sha256_hash (buffer, buffer_size, args, hash_function,
      ...
      case 'p':
        ...
        rc = generate_authorpkcs7 (buffer, buffer_size, args, hash_function,
                                   out_buffer, out_buffer_size);
        break;
      case 'e':
        rc = generate_esl (buffer, buffer_size, args, hash_function, out_buffer, out_buffer_size);
        break;
      default:
  ...
  switch (key)
    {
      ...
      case 't':
        ...
        if (parse_custom_timestamp (args->time, arg))
          {
            prlog (PR_ERR, "ERROR: could not parse given timestamp %s, make sure it is in"
      ...
      case ARGP_KEY_SUCCESS:
        ...
        if (args->help_flag)
          ...
          prlog (PR_ERR, "ERROR: no secure variable name given... use -n <variable_name> "
                 "option\n");
...
guest_generate_command (int argc, char *argv[])
{
  ...
  struct argp_option options[] = {
    ...
    { "append flag", 'a', "APPEND_FLAG", 0, "set append flag, used when generating auth file" },
  ...
  struct argp argp = {
    options, parse_options, "<input_format>:<output_format> -i <input_file> -o <output_file>\n"
